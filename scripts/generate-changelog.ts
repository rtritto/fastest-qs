...
const version = execSync('npm pkg get version').toString().replaceAll('"', '')
...
if (process.env.GITHUB_OUTPUT)
  appendFileSync(
    ...
    `changelog<<${delimiter}${commits.join(EOL.repeat(2))}${EOL}${delimiter}version=${version}${EOL}`
  )
else console.log('No GitHub actions')